...
function getStatus(key, value) {
  ...
  if (safeMin === safeMax && safeMax === 0) {
    // zero-target params (ammonia, nitrite)
...
function initWaterQuality() {
  ...
  function updateParam(key) {
    ...
    if (cursorEl) {
      const total = p.max - p.min;
      const pos = total > 0 ? ((val - p.min) / total) * 100 : 0;
      cursorEl.style.left = `${Math.max(0, Math.min(100, pos))}%`;
    }
...
function calcStocking() {
  ...
  // also update the shrimp slider max
  const cntSlider = document.getElementById('shrimp-cnt-slider');
  if (cntSlider) {
    cntSlider.max = Math.max(guide.max, parseInt(cntSlider.value || 0));
  }
}
